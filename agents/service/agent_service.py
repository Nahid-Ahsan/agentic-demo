...
class TravelAgent:
    ...
    async def process(self, state: AgentState, db):
        history = vectorstore.similarity_search(state["user_id"], k=5)
...
workflow.add_node("flight_agent", lambda state, db=Depends(get_database): flight_agent.process(state, db))
workflow.add_node("hotel_agent", lambda state, db=Depends(get_database): hotel_agent.process(state, db))
...
async def confirm_booking(request, user_id):
    ...
    state = graph.get_state({"configurable": {"thread_id": request.session_id}}).values
